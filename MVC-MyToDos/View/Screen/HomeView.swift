...
class HomeView: UIView {
	...
	private(set) var emptyState = EmptyStateView(frame: .zero, title: "Please 'Add List' to start")
	...
	private(set) var todosList = [ToDosListModel]()
	...
	func setToDosLists(_ lists: [ToDosListModel]) {
		todosList = lists
		tableView.reloadData()
		emptyState.isHidden = todosList.count > 0
	}
}
...
extension HomeView: UITableViewDelegate {
	...
	func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
		delegate?.selectedList(todosList[indexPath.row])
	}
}
...
extension HomeView: UITableViewDataSource {
	...
	func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		cell.setCellParametersForList(todosList[indexPath.row])
		return cell
	}
	
	func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
		if editingStyle == .delete {
			let list = todosList[indexPath.row]
