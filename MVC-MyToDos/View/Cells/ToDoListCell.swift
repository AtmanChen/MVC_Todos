...
class ToDoListCell: UITableViewCell {
    ...
    func setCellParametersForList(_ toDosList: ToDosListModel) {
        let image = UIImage(systemName: toDosList.icon)?
            .withTintColor(.mainBlueColor, renderingMode: .alwaysOriginal)
            .withConfiguration(UIImage.SymbolConfiguration(pointSize: 35.0))
        
        iconView.image = image
        titleLabel.text = toDosList.title
        taskLabel.text = "\(toDosList.todos.count) ToDos"
    }
}
